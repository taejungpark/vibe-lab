...
# Claude Code는 58개 툴 스키마 + 시스템 프롬프트로 시작부터 ~26k 토큰을 보내고,
# 에이전트 세션이 길어지면 prompt가 30k+까지 자람. num_ctx가 작으면 출력 생성 중
# 컨텍스트가 넘쳐 무한 반복(runaway) 생성에 빠지므로 65536으로 설정.
# (출력 토큰은 LiteLLM hook에서 16384로 클램프해 폭주를 추가 차단.)
